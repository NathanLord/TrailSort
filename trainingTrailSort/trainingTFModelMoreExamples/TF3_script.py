...
# ## Intro
# 
# This model changes from the base model in using a larger dataset. I also provided some examples of some testing with the following below saving of the initial model trained on the larger data set.
# 
# Examples:
# 
#     Increased epochs - more full pass throughs
#     Regularizers - help with overfitting, penilize high weights.
#     Dropout - Playing with droput - remove (set to zero) random nodes to help withh overfitting
#     More layers
...
PIL.Image.open(str(black_bears[30]))
...
# Solutions
# 
# - Increase dataset size (artifically using data augmenation if can not get more data)
...
# %% [markdown]
# Again, the training accuracy is increasing linearly over time, whereas validation accuracy stalls around 45% in the training process. Also, the difference in accuracy between training and validation accuracy is noticeable—a sign of [overfitting](https://www.tensorflow.org/tutorials/keras/overfit_and_underfit).
# 
...
# 
# Use *data augmentation*, add *dropout*, and *regularizer*.
...
# %% [markdown]
# Increased levels of random rotation and zoom as well as added contrast in this model training session.
...
# 
# However, validation accuracy is decreased from previous training sessions.
